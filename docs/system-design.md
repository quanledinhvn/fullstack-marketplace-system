...
Admin POST /admin/documents/:id/retry
      │
      ▼
API lookup jobId from DB ──► job.retry() ──► reset attempt count
      │
      ▼
...
### Layer 1 — Worker: before calling external service

```
Worker picks job (may be a stalled job retrying)
      │
      ▼
SELECT status FROM documents WHERE id = documentId
      │
      ├── status = PROCESSING ──► proceed to call external service
      │
      └── status ≠ PROCESSING ──► skip, return
          (already handled by a previous run)
```

### Layer 2 — Webhook: when receiving result from mock service
...
SELECT status FROM documents WHERE verification_id = verificationId
      ...
      │
      └── status ≠ PROCESSING ──► return 200 OK, skip
          (duplicate webhook call)
```

### Why no unique constraint needed?

```
status = PROCESSING is the only guard needed:

  - Stalled job retries      ──► Layer 1 blocks before calling service
  - Duplicate webhook        ──► Layer 2 blocks before updating DB
  - No DB constraint needed  ──► status transition is sufficient guard
```
...
- **PENDING state?** → Removed entirely — from DB enum, code, and shared types. Document is created directly with `PROCESSING` status since upload and enqueue are always atomic within the same request.
