...
export class VerificationConsumer extends WorkerHost {
  ...
  async process(job: Job<{ documentId: string }>, token?: string): Promise<void> {
    ...
    this.logger.log(`[process] jobId=${job.id} documentId=${documentId} attempt=${job.attemptsMade + 1}`);

    const doc = await this.repo.findById(documentId);
    if (!doc || doc.status !== DocumentStatus.PROCESSING) {
      this.logger.warn(`[process] skip jobId=${job.id} documentId=${documentId} status=${doc?.status ?? 'not_found'}`);
      return;
    }

    try {
      const { verificationId, result } = await this.verificationService.callMockService(documentId);
      const nextStatus = result as DocumentStatus;
      await this.repo.updateStatus(documentId, { verificationId, status: nextStatus });
      ...
      this.logger.log(`[process] done jobId=${job.id} documentId=${documentId} verificationId=${verificationId} result=${result}`);
    } catch (err) {
      if (err instanceof RateLimitError) {
        this.logger.warn(`[process] rate-limited jobId=${job.id} documentId=${documentId}, delayed 60s`);
  ...
  async onFailed(job: Job<{ documentId: string }>, err: Error): Promise<void> {
    ...
    this.logger.error(`[failed] jobId=${job.id} documentId=${job.data.documentId} attempt=${job.attemptsMade}/${maxAttempts} err=${err.message}`);
