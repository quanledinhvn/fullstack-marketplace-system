...
export function processAsync(params: {
	...
	console.log(
		`[process] verificationId=${params.verificationId} result=${result} delay=${delayMs}ms`,
	);

	setTimeout(() => {
		console.log(
			`[callback] verificationId=${params.verificationId} documentId=${params.documentId} result=${result} -> ${params.callbackUrl}`,
		);
